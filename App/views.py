...
from .models import Profile,service,Comments,Rating
from django.contrib.auth.decorators import login_required
from .forms import NewserviceForm,NewProfileForm,CommentForm,VotingForm
...
def index(request):
    ones_service = service.objects.all()
    all_services = service.get_all_services()
    return render(request, 'index.html', {"all_services": all_services,"ones_service":ones_service})
...
def my_profile(request):
    ...
    return render(request, 'profile.html', {"my_services":my_services, "my_profile":my_profile})

@login_required(login_url='/accounts/login/') 
def one_service(request,id):

    ones_service = service.objects.filter(id = id)
    ...
    return render(request,'service.html',{"ones_service":ones_service,"all_ratings":all_ratings,"form":form,"usability":average_usability,"design":average_design,"content":average_content})


# @login_required(login_url='/accounts/login/')
def search_service(request):
    if 'service_name' in request.GET and request.GET["service_name"]:
        search_term = request.GET.get("service_name")
        searched_service = service.search_by_title(search_term)
        message = f"{search_term}"
        return render(request, "search.html",{"message":message,"service": searched_service})
    else:
...
def add_comment(request, proj_id):
    current_user = request.user
    service_item = service.objects.filter(id = proj_id).first()
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            comment.commented_service = service_item
            comment.save()
            return redirect('index')
...
def comment(request, id):
    mycomments = Comments.objects.filter(commented_service = id).all()
